...
LevelDefault::LevelDefault(GLFWwindow * window) {
	this->window = window;
}

void LevelDefault::setup() {
	...
	this->sky = Cubemap("resources/skybox/ss_1/", ".png");
	sky.Setup();
	...
	this->projection = glm::perspective(45.f, (GLfloat)WIDTH / (GLfloat)HEIGHT, 0.1f, 1000.0f);
...
void LevelDefault::mainCycle() {
	...
	if (this->spawnTimer > 3.f) {
		spawnTimer = 0;
		if (randi % 4 == 0) {
			Enemy * enemy = new Enemy(40, 50, 0.8f, 10.f, &Models::enemy);
			enemy->pos = ship->pos + vec3(Ray::randFloat(-100.f, 100.f), Ray::randFloat(-100.f, 50.f), Ray::randFloat(500.f, 1200.f));
			EntityController::instance->push(enemy);
		}
		if (randi % 4 == 1) {
			Enemy * enemy = new Enemy(40, 50, 0.8f, 10.f, &Models::enemy1);
			enemy->pos = ship->pos + vec3(Ray::randFloat(-100.f, 100.f), Ray::randFloat(-100.f, 50.f), Ray::randFloat(500.f, 1200.f));
			EntityController::instance->push(enemy);
		}
		if (randi % 4 == 2) {
			Asteroid * asteroid = new Asteroid(ship->pos + vec3(Ray::randFloat(-100.f, 100.f), Ray::randFloat(-100.f, 50.f), Ray::randFloat(500.f, 1200.f)), Ray::randFloat(5.f, 20.f), rand() % 5, 10);
...
void LevelDefault::handleShipMovement() {
